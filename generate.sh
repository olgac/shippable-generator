...
for APLICATION_NAME in $(cat $ROOT_PATH/applications.list); do
    ...
    echo $APLICATION_NAME $LOWER_UNDERSCORE $UPPER_UNDERSCORE

    for TEMPLATE_FILE in $ROOT_PATH/templates/*; do
        sed "s/${LOWER_DASH_KEY}/${APLICATION_NAME}/g; s/${LOWER_UNDERSCORE_KEY}/${LOWER_UNDERSCORE}/g; s/${UPPER_UNDERSCORE_KEY}/${UPPER_UNDERSCORE}/g;" $TEMPLATE_FILE > $ROOT_PATH/$NOW/$APLICATION_NAME/$(basename $TEMPLATE_FILE).txt
    done;
done
